...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); //non-owning
    ...
    //Local chatbot
    //std::cout << "Local chatBot creation." << "\n";
    ChatBot chatBot("../images/chatbot.png"); // Create a new chatbot.
    chatBot.SetChatLogicHandle(this); // both chatbot and chatlogic know about each other (refer).
    
    // add chatbot to graph root node
    chatBot.SetRootNode(rootNode);
    
    //_chatBot = &chatBot; //Assign local chatbot to ChatLogic's chatbot reference.
